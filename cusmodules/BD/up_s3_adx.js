...
function adx(code, filename, io, socket) {
  ...
  FC.writeFileSync('public/BD_temp/adx/'+zone_id+'.html',newHtml);
  ...
  var fi = fs.readFileSync('public/BD_temp/adx/'+zone_id+'.html');
 ...
 s3.upload(params).on('httpUploadProgress', function (evt) {
    ...
    //上傳完畢或是碰到錯誤
    // sendSocket();
    io.sockets.connected[socket.id].emit('StoC BD adx upload ok', {adx: 'https://adx.doublemax.net/' + params.Key});
  });
